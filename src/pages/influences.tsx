...
export default function InfluencesPage({ items }: InfluencesPageProps) {
  const types = ['Person', 'Book', 'Concept', 'Work'] as const
  const groupedItems = types.reduce(
    (acc, type) => {
        ...
        {types.map((type, typeIndex) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-black/40 text-sm mb-6">{typeDescriptions[type]}</p>
                    ...
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          {item.why && (
                            <p className="text-sm text-black/50 mt-1">{item.why}</p>
                          )}
                        </div>
                        {item.link && (
                          <svg
